...
function getRootDeps(template, context) {
  if (production && template.aI_) {
    return template.aI_;
  }

  attachDepsAndComponentsToTemplate(template, context);

  var deps = template.aI_ = Object.keys(template.aJ_).map(key => template.aJ_[key]);
...
function attachDepsAndComponentsToTemplate(target, context) {
  ...
  if (production && target.aJ_) return;
  ...
  if (meta.tags) {
    meta.tags.forEach(tagPath => {
      ...
      if (tag.aJ_) Object.assign(deps, tag.aJ_);
