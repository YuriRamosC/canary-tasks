...
module.exports = function dynamicTag(out, tag, getAttrs, renderBody, args, props, componentDef, key, customEvents) {
  if (tag) {
    ...
    if (typeof tag === "string") {
      ...
      if (renderBody) {
        out.bb_(tag, attrs, key, componentDef, addEvents(componentDef, customEvents, props));
        renderBody(out);
        out.bc_();
      } else {
        out.bd_(tag, attrs, key, componentDef, addEvents(componentDef, customEvents, props));
      }
    } else {
      if (attrs == null) {
      ...
      if (renderer) {
        ...
        if (isFn) {
          ...
          if (!isW10NOOP && isFn) {
            ...
            var parentComponentDef = componentsContext._R_;
            var globalContext = componentsContext.k_;
            componentsContext._R_ = new ComponentDef(component, parentComponentDef.id + "-" + parentComponentDef._M_(key), globalContext);
            ...
            componentsContext._R_ = parentComponentDef;
          }
          out.ef();
        } else {
    ...
    out.bf(key, component, IS_SERVER && componentDef && componentDef._J_ & FLAG_WILL_RERENDER_IN_BROWSER);
    renderBody(out);
    out.ef();
  }
};
...
function attrsToCamelCase(attrs) {
  ...
  for (var key in attrs) {
    result[changeCase.ba_(key)] = attrs[key];
  }
